...
fun HabitEditScreen(
    ...
    habitProgress?.let { progress ->
        if (!isInitialized) {
            name = TextFieldValue(progress.habit.name)
            description = TextFieldValue(progress.habit.description)
            selectedDays = progress.habit.activeDays
            selectedColor = progress.habit.colorHex
            isInitialized = true
        }
    }
    ...
    Scaffold(
        ...
        if (habitProgress == null && !isInitialized) {
            Column(
                modifier = Modifier
                    .fillMaxSize()
            ...
            Column(
                ...
                Button(
                    onClick = {
                        if (name.text.isNotBlank() && habitProgress != null) {
                            scope.launch {
                                habitRepository.updateHabit(
                                    habitProgress!!.habit.copy(
                                        name = name.text.trim(),
